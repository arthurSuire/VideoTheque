...
namespace VideoTheque.Controllers
{
    ...
    public class HostsController : ControllerBase
    {
        ...
        [HttpGet]
        public async Task<List<HostsViewModel>> GetHosts() => (await _hostBusiness.GetHosts()).Adapt<List<HostsViewModel>>();

        [HttpGet("{id}")]
        public async Task<HostsViewModel> GetHost([FromRoute] int id) => _hostBusiness.GetHost(id).Adapt<HostsViewModel>();

        [HttpPost]
        public async Task<IResult> InsertHost([FromBody] HostsViewModel hostVM)
        {
        ...
        [HttpPut("{id}")]
        public async Task<IResult> UpdateHost([FromRoute] int id, [FromBody] HostsViewModel hostVM)
        {
